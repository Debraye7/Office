...
export default function GoalCard({ goal, refresh }:{ goal:TypeGoal, refresh:()=>void }) {
  const { folder, title, description, priority, status, progress, createdAt, updatedAt, dueDate, assignedTo, objectives  } = goal;
    ...
    <>
      <li onClick={()=>setOpenModal(true)} className="flex flex-col gap-3 h-full bg-secondary-light dark:bg-secondary-dark hover:bg-transparent rounded-xl py-4 border border-tertiary-light dark:border-tertiary-dark shadow-md shadow-quaternary dark:shadow-secondary-dark cursor-pointer duration-300">
        <section className="flex flex-wrap md:flex-nowrap justify-between gap-x-4 gap-y-2 px-4">
          <span className="h-fit px-2 sm:px-4 py-0.5 rounded bg-blue-light/20 dark:bg-blue-dark/20">
          ...
          <div className="flex flex-wrap md:flex-nowrap items-end gap-1">
            <span className={`h-fit px-2 sm:px-4 py-0.5 rounded text-nowrap font-semibold text-xs ${getStatusTagColor()}`}>{status}</span>
            <span className={`h-fit px-2 sm:px-4 py-0.5 rounded text-nowrap font-semibold text-xs ${getPriorityTagColor()}`}>{priority}</span>
          </div>
        </section>
